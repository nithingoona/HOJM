...
def prepare(size):
    return sparse.random(size, size, density=1, format='csr'), sparse.random(size, 1, density=1, format='csr')

def compute_while_training(A,B):
    I = sparse.identity(A.shape[0], format=A.format)
    A0 = A + 2*I
    D_flat = A0.diagonal()
...
def main():
    ...
    with open('scipy_HOM_SLE_dense.json', 'w') as f:
        json.dump(results, f)
