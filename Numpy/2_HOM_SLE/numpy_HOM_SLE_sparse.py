...
def prepare(size):
    ...
    A = np.zeros([total_number_of_nodes_in_2_dimensions,total_number_of_nodes_in_2_dimensions])
    B = np.zeros([total_number_of_nodes_in_2_dimensions,1])
    ...
    for i in range(number_of_nodes_in_1_dimension):
        ...
        A[node,node] = 1
        j = number_of_nodes_in_1_dimension-1
        node = i + j*number_of_nodes_in_1_dimension
        A[node,node] = 1
    ...
    for j in range(number_of_nodes_in_1_dimension):
        ...
        node = i + j*number_of_nodes_in_1_dimension
        A[node,node] = 1
        i = number_of_nodes_in_1_dimension-1
        node = i + j*number_of_nodes_in_1_dimension
        A[node,node] = 1
    ...
    for i in range(1,number_of_nodes_in_1_dimension-1):
        for j in range(1,number_of_nodes_in_1_dimension-1):
            ...
            A[node,node+1] = -0.25
            A[node,node-1] = -0.25
            A[node,node+number_of_nodes_in_1_dimension] = -0.25
            A[node,node-number_of_nodes_in_1_dimension] = -0.25
    B[int(number_of_nodes_in_1_dimension/2)+number_of_nodes_in_1_dimension*int(number_of_nodes_in_1_dimension/2)] = 1
...
def main():
    ...
    with open('numpy_HOM_SLE_sparse.json', 'w') as f:
        json.dump(results, f)
