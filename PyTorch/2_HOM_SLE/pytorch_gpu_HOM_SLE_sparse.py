...
def prepare(size):
    ...
    indices = torch.tensor(indices_list, dtype=torch.long).t().cuda()
    values = torch.tensor(values_list, dtype=torch.float).cuda()
    ...
    #                             1),
    #                         device='cuda')
    B = torch.randn(total_number_of_nodes_in_2_dimensions, 1, device='cuda')
    A = A.to_dense()

    return A, B

def compute_while_training(A,B):
    I = torch.eye(A.shape[0],device='cuda')
    A0 = A + 2*I
    D_flat = A0.diagonal()
    ...
    for i in range(num_iter):
        Alpha0 = Alpha - I
        _ = torch.matmul(Alpha0,Beta)
        torch.cuda.synchronize()
        Beta = - torch.matmul(Alpha0,Beta)
        torch.cuda.synchronize()
        _ = torch.matmul(Alpha,Alpha)
        torch.cuda.synchronize()
        Alpha = - torch.matmul(Alpha,Alpha)
        torch.cuda.synchronize()
        _ = torch.matmul(A, B)
        torch.cuda.synchronize()
        C = - torch.matmul(Alpha,C)
        torch.cuda.synchronize()
        C = C + Beta
...
def main():
    ...
    with open('pytorch_gpu_HOM_SLE_sparse.json', 'w') as f:
        json.dump(results, f)
